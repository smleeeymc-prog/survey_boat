...
// 키워드 → 색상. 지도(style.js)의 캐빈 틴트만 쓴다.
// 설문 화면은 더 이상 키워드로 캐빈 색을 바꾸지 않는다 — 키워드는 배에 나타나는 요소로만
// 표현한다(glb-nodes.js KEYWORD_NODES). 지도도 같은 규칙으로 맞출지는 지도 쪽 결정 대기.
...
// 키워드 → 지도의 배에 붙는 소품. 지도(style.js·fleet.js)만 쓴다 — 지도는 갈매기·튜브 두 가지만
// 인스턴싱한다. 설문 화면의 전체 매핑은 glb-nodes.js KEYWORD_NODES 이고, 이 표는 그 부분집합이다.
// (튜브는 예전엔 '관계'였는데 KEYWORD_NODES를 따라 '가족'으로 옮겼다. '관계'는 이제 Lamp)
